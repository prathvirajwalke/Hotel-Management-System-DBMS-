...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
   ...
   $sql="SELECT `username`, `password` FROM `customer` WHERE username='{$_SESSION['username']}' and password='$md_pass'";
   ...
   if($result=mysqli_query($conn,$sql))
	 {
		 ...
		 if($_SESSION["username"]===$row["username"])
		 {
			...
			window.location=\"menu1.php\"</script>";
			exit();
	 }
   else {
...
<!DOCTYPE html>
  ...
  <script>
	...
	<input type="submit" value="Register" class="btn" formaction="Register.php" method="get"/></td>
	</tr>
	</table>
</form>
	</div>
